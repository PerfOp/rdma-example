 ...
 */
// ret = rdma_create_id(rdmaServer.cm_event_channel, &rdmaServer.cm_server_id,
// NULL, RDMA_PS_TCP); if (ret) { rdma_error("Creating server cm id failed with
// errno: %d ", -errno); return -errno;
// }
// debug("A RDMA connection id for the server is created \n");
// [> Explicit binding of rdma cm id to the socket credentials <]
// ret = rdma_bind_addr(rdmaServer.cm_server_id, (struct sockaddr*)
// server_addr); if (ret) { rdma_error("Failed to bind server address, errno: %d
// \n", -errno); return -errno;
// }
...
// ret = rdma_listen(rdmaServer.cm_server_id, 8); [> backlog = 8 clients, same
// as TCP, see man listen<] if (ret) { rdma_error("rdma_listen failed to listen
// on server address, errno: %d ", -errno);
// return -errno;
// }
...
// rdmaServer.cm_client_id); return ret;
// }

...
/* When we call this function rdmaServer.cm_client_id must be set to a valid identifier.
...
// {
	...
	// rdmaServer.cm_event_channel = rdma_create_event_channel();
	// if (!rdmaServer.cm_event_channel) {
		// rdma_error("Creating cm event channel failed with errno : (%d)", -errno);
		// return -errno;
	// }
	// debug("RDMA CM event channel is created successfully at %p \n",
			// rdmaServer.cm_event_channel);
	/* rdma_cm_id is the connection identifier (like socket) which is used
	 * to define an RDMA connection.
	 */
	// ret = rdma_create_id(rdmaServer.cm_event_channel, &rdmaServer.cm_server_id, NULL, RDMA_PS_TCP);
	// if (ret) {
	...
	// ret = rdma_bind_addr(rdmaServer.cm_server_id, (struct sockaddr*) server_addr);
	// if (ret) {
	...
	// ret = rdma_listen(rdmaServer.cm_server_id, 8); [> backlog = 8 clients, same as TCP, see man listen<]
	// if (ret) {
	 ...
	 * the connect event.
	 */
	// ret = process_rdma_cm_event(rdmaServer.cm_event_channel,
	 ...
	 */
	// rdmaServer.cm_client_id = cm_event->id;
...
int RdmaServer::start_rdma_server(struct sockaddr_in *server_addr)
{
	 ...
	 * to define an RDMA connection.
	 */
	ret = rdma_create_id(this->cm_event_channel, &this->cm_server_id, NULL, RDMA_PS_TCP);
	if (ret) {
		rdma_error("Creating server cm id failed with errno: %d ", -errno);
		return -errno;
	}
	 ...
	 * RDMA CM event channel from where the listening id was created. Here we
	 * have only one channel, so it is easy. */
	ret = rdma_listen(this->cm_server_id, 8); /* backlog = 8 clients, same as TCP, see man listen*/
	if (ret) {
		rdma_error("rdma_listen failed to listen on server address, errno: %d ",
				-errno);
		return -errno;
...
int RdmaServer::accept_client_connection()
{
	...
	/* we prepare the receive buffer in which we will receive the client metadata*/
        this->client_metadata_mr = rdma_buffer_register(this->pd /* which protection domain */,
	...
	this->client_recv_sge.addr = (uint64_t) this->client_metadata_mr->addr; // same as &client_buffer_attr
...
int RdmaServer::send_server_metadata_to_client()
{
	...
	* do RDMA READs and WRITEs. */
       this->server_buffer_mr = rdma_buffer_alloc(this->pd /* which protection domain */,
	...
	*/
       ...
       this->server_metadata_attr.length = (uint32_t) this->server_buffer_mr->length;
       this->server_metadata_attr.stag.local_stag = (uint32_t) this->server_buffer_mr->lkey;
       this->server_metadata_mr = rdma_buffer_register(this->pd /* which protection domain*/,
	...
	*/
       ...
       this->server_send_wr.send_flags = IBV_SEND_SIGNALED; // We want to get notification
       /* This is a fast data path operation. Posting an I/O request */
       ret = ibv_post_send(this->client_qp /* which QP */,
		       &this->server_send_wr /* Send request that we prepared before */,
		       &this->bad_server_send_wr /* In case of error, this will contain failed requests */);
       if (ret) {
